...
function ensureConnectionIsEncrypted(sequelize) {
    ...
    .catch((err) => {
        if (err.message === 'self signed certificate in certificate chain') {
            logger.error(`Sequelize is throwing error "${err.message}", which it does seemingly any time the certificate is invalid. Ensure your AUTH_SERVICE_PG_CA_CERT is set correctly.`);
        } else {
